...
export async function sendMail(data: MailData) {
  ...
  const isMock = process.env.MAIL_CONSOLE_MOCK === 'true'

  const payload: MailPayload = {
    template_id: data.templateId || defaultTemplateId || '',
    to: data.to || process.env.ADMIN_EMAIL || 'admin@example.com',
    variables: data.variables
  }

  if (isMock) {
    console.log('📧 [Mock] Mail payload:', payload)
    return { success: true, status: 'ok', mock: true, data: payload }
  }

  if (!apiKey) {
    console.error('MAIL_CONSOLE_API_KEY is not configured')
